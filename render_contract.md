...
**Status:** FROZEN  
**Date:** 2026-04-12  
**Scan basis:** `review/finalpool-youtube-repo-11` branch — post-pilot-changeset snapshot  
**Prior contract:** v1.0.0 (2026-04-09, `review/render-artifact`) — schema unchanged; scan basis updated.
...
Examples: `"module_a.py"`, `"tests/test_module_a.py"`, `".python_tmp/setup_env.py"`.
...
out at the moment of the scan. Files in hidden directories (e.g.,
`.python_tmp/`) are included in the scope.

Files with no qualifying TODO lines are listed in `scanned_files` only if
they were explicitly included in the scan pass. The convention for this
repository excludes the `.python_tmp/` utility scripts from the reported
`scanned_files` list (they carry no application-level TODOs); the four
application-code paths are the canonical scope:
...
|-------|--------------------------------------|----------------|
| `tag` | No `(tag)` token in source line | `null` |
...
For this rendering cycle the target branch is `review/finalpool-youtube-repo-11`,
branched from `main` and augmented with the pilot code changes
(4 TODOs resolved in `module_a.py`; all 3 TODOs resolved in `utils/helpers.py`).
